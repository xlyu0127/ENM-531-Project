...
model.load_state_dict(torch.load('r4e'))
...
#Remove all atom that does not directly attaches a hydroden. In practice it is set directly to -1 independent of the environment vector.
Xcl = X[(Y > 0).squeeze(), :]
Ycl = Y[(Y > 0).squeeze(), :]

torch.save(Xcl,'r4traincode')
torch.save(Ycl, 'r4trainY')
...
torch.save(Xtestcl,'r4testcode')
torch.save(Ytestcl, 'r4testY')
total_loss = 0
